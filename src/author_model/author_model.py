...
class BMAChatAssistant(dspy.Signature):
    ...
    citations: Citations = dspy.OutputField(desc="Citations with quoted text and source references from the response.")
...
class RAG(dspy.Module):
    ...
    def _create_citations_from_passages(self, passages):
        """
        Convert retrieval passages to Citations format.
        
        Args:
            passages: List of passage dictionaries from DatabricksRM
            
        Returns:
            Citations object with source references
        """
        citation_objects = []
        ...
        for idx, passage in enumerate(passages):
            ...
            citation_objects.append(Citations.Citation(
                cited_text=text_preview,
                document_title=f"Document: {path} | Doc ID: {doc_id}",
                document_index=idx,
                start_char_index=0,
                end_char_index=len(text_preview)
            ))
        
        return Citations(citations=citation_objects)
    ...
    def forward(self, question):
        ...
        retrieval_result = self.retriever(question_text)
        
        # Extract passages for citation construction
        passages = retrieval_result if isinstance(retrieval_result, list) else getattr(retrieval_result, 'passages', [])
        context = str(retrieval_result) if retrieval_result else ""

        with dspy.context(lm=self.lm):
            response = self.response_generator(context=context, question=question_text, history=history)
            
            # Add citations from retrieved sources
            if passages:
                citations = self._create_citations_from_passages(passages)
                response = dspy.Prediction(response=response.response, citations=citations)
...
# MAGIC %md
# MAGIC ## Test RAG with Citations

# COMMAND ----------

# Test example
test_result = rag({'messages': [{'content': 'What was the overall audit rating for CTCBL?', 'role': 'user'}]})

print(f"Response: {test_result.response}\n")

# Display citations
if hasattr(test_result, 'citations') and test_result.citations:
    citations_list = test_result.citations.citations
    print(f"Citations: {len(citations_list)} source(s)\n")
    for idx, citation in enumerate(citations_list, 1):
        print(f"[{idx}] {citation.document_title}")
        print(f"    {citation.cited_text[:150]}...\n")
else:
...
# MAGIC %md
# MAGIC ## Citations Implementation
# MAGIC 
# MAGIC The RAG system automatically provides citations from retrieval sources, showing which documents informed each answer.
# MAGIC 
# MAGIC **How it works:**
# MAGIC 1. Retrieves relevant documents using DatabricksRM
# MAGIC 2. Generates response based on retrieved context
# MAGIC 3. Attaches citations with source references and text excerpts
# MAGIC 
# MAGIC **Usage Example:**
# MAGIC ```python
# MAGIC result = rag({'messages': [{'content': 'Your question?', 'role': 'user'}]})
# MAGIC print(result.response)
# MAGIC for citation in result.citations.citations:
# MAGIC     print(f"{citation.document_title}: {citation.cited_text}")
# MAGIC ```
